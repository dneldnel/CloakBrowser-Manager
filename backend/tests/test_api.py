...
def test_launch_failure_500(app_client: TestClient):
    ...
    pid = create.json()["id"]
    main.browser_mgr.launch = AsyncMock(side_effect=RuntimeError("local display unavailable"))
    resp = app_client.post(f"/api/profiles/{pid}/launch")
    assert resp.status_code == 500
    assert resp.json()["detail"] == "local display unavailable"
...
def test_ws_allows_same_origin(app_client: TestClient):
    ...
    # Same-origin passes Origin check. CDP then fails because no real browser
    # is listening, but any error other than 4403 means Origin passed.
    try:
        with app_client.websocket_connect(
            f"/api/profiles/{pid}/cdp",
